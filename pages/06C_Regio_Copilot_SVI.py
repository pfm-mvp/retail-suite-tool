...
def collapse_to_daily_store(df: pd.DataFrame, store_key_col: str) -> pd.DataFrame:
    ...
    # recompute core derived metrics robustly (keep sales_per_sqm as-is; we compute it explicitly elsewhere too)
...
def nice_component_name(key: str) -> str:
    mapping = {
        ...
        "avg_basket_size": "ATV (gem. bon)",
        "sales_per_transaction": "Sales / transactie (proxy)",
    }
...
def build_opportunities_v2_one_lever(
    ...
    for _, r in g.iterrows():
        ...
        if pd.notna(conv_target) and pd.notna(conv) and conv < conv_target and foot > 0 and pd.notna(atv):
            ...
            driver = f"Conversie → regio-median (+{delta_pp:.1f}pp)"
...
def compute_daily_baselines(
    ...
    """
    Returns dict with:
      - store_sqm_map (shop_id -> sqm_effective)
      - region_sqm_sum
      - company_sqm_sum
      - region_daily (date, turnover, footfall, transactions)
      - company_daily (date, turnover, footfall, transactions)
    """
    d = df_daily_store.copy()
    d["date"] = pd.to_datetime(d["date"], errors="coerce")
    d = d.dropna(subset=["date"])
    ...
    # sqm map (from merged)
    sqm_map = {}
    if merged is not None and not merged.empty and "id" in merged.columns and "sqm_effective" in merged.columns:
        tmp = merged[["id", "sqm_effective"]].drop_duplicates().copy()
        tmp["id"] = pd.to_numeric(tmp["id"], errors="coerce")
        tmp["sqm_effective"] = pd.to_numeric(tmp["sqm_effective"], errors="coerce")
        tmp = tmp.dropna(subset=["id", "sqm_effective"])
        sqm_map = dict(zip(tmp["id"].astype(int).tolist(), tmp["sqm_effective"].astype(float).tolist()))

    # daily totals
    gcols = ["date"]
    reg = d[d["region"] == region_choice].copy()
    reg_daily = reg.groupby(gcols, as_index=False).agg(
        turnover=("turnover", "sum"),
        footfall=("footfall", "sum"),
        transactions=("transactions", "sum"),
    )

    comp_daily = d.groupby(gcols, as_index=False).agg(
        turnover=("turnover", "sum"),
        footfall=("footfall", "sum"),
        transactions=("transactions", "sum"),
    )

    # sqm sums (constant denominator for sales/m² baseline; sum of sqm across stores)
    reg_ids = []
    if merged is not None and not merged.empty and "region" in merged.columns:
        reg_ids = merged[merged["region"] == region_choice]["id"].dropna().astype(int).unique().tolist()
    company_ids = merged["id"].dropna().astype(int).unique().tolist() if merged is not None and not merged.empty else []

    region_sqm_sum = float(np.nansum([sqm_map.get(i, np.nan) for i in reg_ids])) if reg_ids else np.nan
    company_sqm_sum = float(np.nansum([sqm_map.get(i, np.nan) for i in company_ids])) if company_ids else np.nan

    if region_sqm_sum == 0:
        region_sqm_sum = np.nan
    if company_sqm_sum == 0:
        company_sqm_sum = np.nan

    return {
        "sqm_map": sqm_map,
        "region_sqm_sum": region_sqm_sum,
        "company_sqm_sum": company_sqm_sum,
        "region_daily": reg_daily,
        "company_daily": comp_daily,
    }
...
def build_trend_series(
    ...
    """
    metric_key in:
      turnover, footfall, transactions, conversion_rate, sales_per_visitor, avg_basket_size, sales_per_sqm
    Builds long dataframe: date, series(Store/Regio/Company), value
    """
    d = df_daily_store.copy()
    d["date"] = pd.to_datetime(d["date"], errors="coerce")
    d = d.dropna(subset=["date"])
    ...
    base = compute_daily_baselines(d, merged, store_key_col, region_choice)
    reg_daily = base["region_daily"].copy()
    comp_daily = base["company_daily"].copy()
    sqm_map = base["sqm_map"]
    reg_sqm_sum = base["region_sqm_sum"]
    comp_sqm_sum = base["company_sqm_sum"]
    store_sqm = float(sqm_map.get(int(store_id), np.nan))

    store = d[pd.to_numeric(d[store_key_col], errors="coerce").astype("Int64") == int(store_id)].copy()
    store_daily = store.groupby(["date"], as_index=False).agg(
        turnover=("turnover", "sum"),
        footfall=("footfall", "sum"),
        transactions=("transactions", "sum"),
    )

    def _compute_metric(df_agg: pd.DataFrame, denom_sqm: float | None = None) -> pd.Series:
        t = _safe_num(df_agg.get("turnover", np.nan))
        f = _safe_num(df_agg.get("footfall", np.nan))
        tr = _safe_num(df_agg.get("transactions", np.nan))

        if metric_key == "turnover":
            return t
        if metric_key == "footfall":
            return f
        if metric_key == "transactions":
            return tr
        if metric_key == "conversion_rate":
            return np.where(f > 0, (tr / f) * 100.0, np.nan)
        if metric_key == "sales_per_visitor":
            return np.where(f > 0, (t / f), np.nan)
        if metric_key == "avg_basket_size":
            return np.where(tr > 0, (t / tr), np.nan)
        if metric_key == "sales_per_sqm":
            if denom_sqm is None or pd.isna(denom_sqm) or denom_sqm <= 0:
                return pd.Series(np.nan, index=df_agg.index)
            return np.where(t.notna(), (t / float(denom_sqm)), np.nan)

        return pd.Series(np.nan, index=df_agg.index)

    store_daily["value"] = _compute_metric(store_daily, denom_sqm=store_sqm)
    reg_daily["value"] = _compute_metric(reg_daily, denom_sqm=reg_sqm_sum)
    comp_daily["value"] = _compute_metric(comp_daily, denom_sqm=comp_sqm_sum)

    store_long = store_daily[["date", "value"]].copy()
    store_long["series"] = "Store"

    reg_long = reg_daily[["date", "value"]].copy()
    reg_long["series"] = "Regio"

    comp_long = comp_daily[["date", "value"]].copy()
    comp_long["series"] = "Company"

    out = pd.concat([store_long, reg_long, comp_long], ignore_index=True)
    out = out.dropna(subset=["date"])
    return out
...
def lever_scan(
    ...
    region_choice: str,
    store_id: int,
    compare_to: str = "region",  # "region" or "company"
) -> pd.DataFrame:
    """
    Produces 4 rows: SPV, Conversion, ATV, Sales/m²
    Includes:
      - score_0_100 (clipped ratio mapping)
      - ratio_vs_benchmark
      - gap_pct (benchmark - store) / benchmark * 100  (positive = underperform)
      - store_value, bench_value
    """
    d = df_daily_store.copy()
    d["date"] = pd.to_datetime(d["date"], errors="coerce")
    d = d.dropna(subset=["date"])
    ...
    store = d[pd.to_numeric(d[store_key_col], errors="coerce").astype("Int64") == int(store_id)].copy()
    store_turn = float(_safe_num(store.get("turnover", 0)).fillna(0).sum())
    store_foot = float(_safe_num(store.get("footfall", 0)).fillna(0).sum())
    store_tran = float(_safe_num(store.get("transactions", 0)).fillna(0).sum())

    store_conv = (store_tran / store_foot * 100.0) if store_foot > 0 else np.nan
    store_spv = (store_turn / store_foot) if store_foot > 0 else np.nan
    store_atv = (store_turn / store_tran) if store_tran > 0 else np.nan
    store_sqm_metric = (store_turn / store_sqm) if (pd.notna(store_sqm) and store_sqm > 0) else np.nan

    if compare_to == "region":
        ref = d[d["region"] == region_choice].copy()
        ref_sqm = reg_sqm_sum
    else:
        ref = d.copy()
        ref_sqm = comp_sqm_sum

    ref_turn = float(_safe_num(ref.get("turnover", 0)).fillna(0).sum())
    ref_foot = float(_safe_num(ref.get("footfall", 0)).fillna(0).sum())
    ref_tran = float(_safe_num(ref.get("transactions", 0)).fillna(0).sum())

    ref_conv = (ref_tran / ref_foot * 100.0) if ref_foot > 0 else np.nan
    ref_spv = (ref_turn / ref_foot) if ref_foot > 0 else np.nan
    ref_atv = (ref_turn / ref_tran) if ref_tran > 0 else np.nan
    ref_sqm_metric = (ref_turn / ref_sqm) if (pd.notna(ref_sqm) and ref_sqm > 0) else np.nan

    rows = [
        ("SPV (€ / visitor)", store_spv, ref_spv),
        ("Conversie (%)", store_conv, ref_conv),
        ("ATV (€)", store_atv, ref_atv),
        ("Sales / m² (€)", store_sqm_metric, ref_sqm_metric),
    ]

    out_rows = []
    for name, v, b in rows:
        if pd.notna(v) and pd.notna(b) and float(b) != 0.0:
            ratio = float(v) / float(b) * 100.0
            gap_pct = (float(b) - float(v)) / float(b) * 100.0  # + = underperform
        else:
            ratio = np.nan
            gap_pct = np.nan

        score = _ratio_to_score_0_100(ratio)  # clipped; now we ALSO show ratio & gap explicitly
        out_rows.append({
            "metric": name,
            "store_value": v,
            "benchmark_value": b,
            "ratio_vs_benchmark": ratio,
            "gap_vs_benchmark_pct": gap_pct,
            "score_0_100": score,
        })
...
def main():
        ...
        st.markdown(
            ...
            <div class="pfm-header">
              <div>
                ...
                <div class="pfm-sub">Regio-level: explainable SVI + opportunities + store drilldown + macro context</div>
              </div>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    fetch_ids = all_shop_ids  # company benchmark correctness

    # Metrics
    metric_map = {
        "count_in": "footfall",
        "turnover": "turnover",
        "transactions": "transactions",  # <- dit is de key/data_output voor transacties
    ...
    # ----------------------
    # ✅ Session state: analyse blijft staan bij drilldown select
    # ----------------------
    ...
    if run_btn:
        ...
        st.session_state.store_key_col = store_key_col
        st.session_state.merged = merged2
        st.session_state.df_norm = df_norm
        st.session_state.df_daily_store = df_daily_store
        st.session_state.df_region_daily = df_region_daily
        st.session_state.sqm_col_detected = sqm_col
        st.session_state.params_preview = params_preview
    ...
    # Pull from state
    store_key_col = st.session_state.store_key_col
    merged = st.session_state.merged
    df_norm = st.session_state.df_norm
    df_daily_store = st.session_state.df_daily_store
    df_region_daily = st.session_state.df_region_daily
    sqm_col = st.session_state.sqm_col_detected
    params_preview = st.session_state.params_preview

    # ----------------------
    # Region headline KPI's
    # ----------------------
    foot_total = float(pd.to_numeric(df_region_daily["footfall"], errors="coerce").fillna(0).sum()) if "footfall" in df_region_daily.columns else 0.0
    turn_total = float(pd.to_numeric(df_region_daily["turnover"], errors="coerce").fillna(0).sum()) if "turnover" in df_region_daily.columns else 0.0
    trans_total = float(pd.to_numeric(df_region_daily["transactions"], errors="coerce").fillna(0).sum()) if "transactions" in df_region_daily.columns else 0.0
    ...
    # ----------------------
    # Capture weekly chart data (Pathzz)
    # ----------------------
    ...
    with k3:
        kpi_card("Conversie", fmt_pct(conv), "Transacties / bezoekers")
    with k4:
    ...
    st.markdown(
        "<div class='muted'>Company baseline = alle shops in company (zelfde periode). Regio baseline = shops binnen de gekozen regio. "
        "Baselines voor ratio-KPI’s worden berekend op <b>totalen</b> (dus gewogen), niet als simpele gemiddelden.</div>",
        unsafe_allow_html=True
    )
    ...
    with r2_c:
        ...
        if comp_df is None or comp_df.empty or comp_df["score_0_100"].dropna().empty:
            ...
            bar = (
                ...
                .encode(
                    ...
                    tooltip=[
                        ...
                        alt.Tooltip("ratio:Q", title="vs company (%)", format=".1f"),
                    ],
                )
            ...
            st.caption("Let op: score is geclipt (70–130% → 0–100) voor stabiliteit. Ratio (%) staat in de tooltip.")
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    st.markdown(
        ...
        waarbij per winkel <b>één dominante hefboom</b> wordt gesimuleerd (conversie óf SPV).
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with opp_panel_right:
        ...
        st.markdown(
            ...
            <ul>
              <li>Wijst <b>prioriteit</b> aan (waar zit de hefboom?)</li>
              <li><b>Conservatief</b>: per winkel één driver</li>
              <li><b>Herhaalbaar</b>: zelfde logica per periode</li>
            </ul>
            </div>
            """,
            unsafe_allow_html=True,
    ...
    # ----------------------
    # ✅ Store drilldown (blijft staan zonder re-analyse)
    # ----------------------
    ...
    region_stores["id"] = region_stores["id"].astype(int)
    region_stores = region_stores.sort_values("store_display")

    store_options = region_stores.apply(lambda r: f"{r['store_display']}  ·  storeID {int(r['id'])}", axis=1).tolist()

    # default selection stable
    if "drill_store_label" not in st.session_state:
        st.session_state.drill_store_label = store_options[0] if store_options else None

    st.session_state.drill_store_label = st.selectbox("Winkel", store_options, index=store_options.index(st.session_state.drill_store_label) if st.session_state.drill_store_label in store_options else 0)

    # parse store_id
    selected_store_id = int(str(st.session_state.drill_store_label).split("storeID")[-1].strip())

    store_name = region_stores.loc[region_stores["id"] == selected_store_id, "store_display"]
    store_name = store_name.iloc[0] if not store_name.empty else str(selected_store_id)

    st.markdown(f"### {store_name} · storeID {selected_store_id}")

    ds = df_daily_store.copy()
    ds["date"] = pd.to_datetime(ds["date"], errors="coerce")
    ds = ds.dropna(subset=["date"])

    store_df = ds[pd.to_numeric(ds[store_key_col], errors="coerce").astype("Int64") == selected_store_id].copy()

    store_foot = float(pd.to_numeric(store_df.get("footfall", 0), errors="coerce").fillna(0).sum())
    store_turn = float(pd.to_numeric(store_df.get("turnover", 0), errors="coerce").fillna(0).sum())
    store_tran = float(pd.to_numeric(store_df.get("transactions", 0), errors="coerce").fillna(0).sum())
    store_conv = (store_tran / store_foot * 100.0) if store_foot > 0 else np.nan

    # ✅ Sales/m² altijd: turnover / sqm_effective (niet afhankelijk van API kolom)
    sqm_eff = np.nan
    mrow = merged.loc[merged["id"].astype("Int64") == selected_store_id, "sqm_effective"]
    if not mrow.empty:
        sqm_eff = float(pd.to_numeric(mrow.iloc[0], errors="coerce"))
    store_sales_sqm = (store_turn / sqm_eff) if (pd.notna(sqm_eff) and sqm_eff > 0) else np.nan

    s1, s2, s3, s4 = st.columns([1, 1, 1, 1])
    with s1:
        kpi_card("Footfall", fmt_int(store_foot), "Store · periode")
    with s2:
        kpi_card("Omzet", fmt_eur(store_turn), "Store · periode")
    with s3:
        kpi_card("Conversie", fmt_pct(store_conv), "Store · periode")
    with s4:
        kpi_card("Sales / m²", fmt_eur(store_sales_sqm), "Store · periode (sqm_effective)")

    st.caption("Baselines: Regio/Company zijn gewogen (totalen) per dag; dus eerlijk bij verschillende store sizes.")
    ...
    metric_key = st.selectbox("Metric", list(metric_labels.keys()), format_func=lambda k: metric_labels[k], index=0)

    trend = build_trend_series(
        df_daily_store=df_daily_store,
        merged=merged,
        store_key_col=store_key_col,
        region_choice=region_choice,
    ...
    if trend.empty or trend["value"].dropna().empty:
        st.info("Geen trenddata beschikbaar voor deze selectie/metric.")
    else:
        # Formatting
        y_title = metric_labels.get(metric_key, metric_key)
        if metric_key in ("conversion_rate",):
            fmt = ".1f"
        elif metric_key in ("footfall", "transactions"):
            fmt = ",.0f"
        else:
            fmt = ",.2f"

        color_scale = alt.Scale(domain=["Store", "Regio", "Company"], range=[PFM_PURPLE, PFM_LINE, PFM_RED])
        ...
        chart = (
            ...
            .encode(
                ...
                tooltip=[
                    ...
                    alt.Tooltip("series:N", title="Reeks"),
                    alt.Tooltip("value:Q", title="Waarde", format=fmt),
                ],
            )
    ...
    # Best / worst days (filter out 0/empty)
    ...
    rank_key = st.selectbox("Rank op", list(rank_opts.keys()), format_func=lambda k: rank_opts[k], index=0)

    dday = store_df.copy()
    dday["date"] = pd.to_datetime(dday["date"], errors="coerce")
    dday = dday.dropna(subset=["date"])
    dday["turnover"] = pd.to_numeric(dday.get("turnover", np.nan), errors="coerce")
    dday["footfall"] = pd.to_numeric(dday.get("footfall", np.nan), errors="coerce")
    dday["transactions"] = pd.to_numeric(dday.get("transactions", np.nan), errors="coerce")

    # derived daily
    dday["conversion_rate"] = np.where(dday["footfall"] > 0, dday["transactions"] / dday["footfall"] * 100.0, np.nan)
    dday["sales_per_visitor"] = np.where(dday["footfall"] > 0, dday["turnover"] / dday["footfall"], np.nan)
    dday["avg_basket_size"] = np.where(dday["transactions"] > 0, dday["turnover"] / dday["transactions"], np.nan)
    dday["sales_per_sqm"] = np.where((pd.notna(sqm_eff) and sqm_eff > 0), dday["turnover"] / float(sqm_eff), np.nan)

    # filter: remove dead rows (all zero)
    dday = dday[(dday["turnover"].fillna(0) > 0) | (dday["footfall"].fillna(0) > 0) | (dday["transactions"].fillna(0) > 0)].copy()

    cols_show = ["date", "turnover", "footfall", "transactions", "conversion_rate", "sales_per_visitor", "avg_basket_size", "sales_per_sqm"]

    if dday.empty or dday[rank_key].dropna().empty:
        st.info("Geen dagen met betekenisvolle data gevonden.")
    else:
        top5 = dday.sort_values(rank_key, ascending=False).head(5)[cols_show].copy()
        bot5 = dday.sort_values(rank_key, ascending=True).head(5)[cols_show].copy()

        # pretty format in dataframe
        def _fmt_df(xdf: pd.DataFrame) -> pd.DataFrame:
            o = xdf.copy()
            o["date"] = o["date"].dt.date.astype(str)
            for c in ["turnover"]:
                o[c] = o[c].map(lambda v: fmt_eur_2(v) if pd.notna(v) else "-")
            for c in ["footfall", "transactions"]:
                o[c] = o[c].map(lambda v: fmt_int(v) if pd.notna(v) else "-")
            for c in ["conversion_rate"]:
                o[c] = o[c].map(lambda v: fmt_pct(v) if pd.notna(v) else "-")
            for c in ["sales_per_visitor", "avg_basket_size", "sales_per_sqm"]:
                o[c] = o[c].map(lambda v: fmt_eur_2(v) if pd.notna(v) else "-")
            return o
        ...
        with cL:
            ...
            st.dataframe(_fmt_df(top5), use_container_width=True, hide_index=True)
        with cR:
            st.subheader("Bottom 5 days")
            st.dataframe(_fmt_df(bot5), use_container_width=True, hide_index=True)

    st.markdown("</div>", unsafe_allow_html=True)

    # Lever scan (now not misleading: show ratio & gap)
    st.markdown('<div class="panel"><div class="panel-title">Lever scan — waar zit het gat?</div>', unsafe_allow_html=True)

    compare_to = st.radio("Vergelijking", ["vs Regio", "vs Company"], horizontal=True)
    comp_key = "region" if compare_to == "vs Regio" else "company"
    ...
    if ldf.empty or ldf["score_0_100"].dropna().empty:
        st.info("Geen lever scan data beschikbaar.")
    else:
        # Build a better bar: score + tooltip ratio/gap
        plot = ldf.copy()
        plot["score"] = pd.to_numeric(plot["score_0_100"], errors="coerce")
        plot["ratio"] = pd.to_numeric(plot["ratio_vs_benchmark"], errors="coerce")
        plot["gap"] = pd.to_numeric(plot["gap_vs_benchmark_pct"], errors="coerce")

        # Explanation line: score can be 100 even if above benchmark; ratio & gap show reality
        st.caption("Score is geclipt (70–130% → 0–100). Daarom kan 100 ook betekenen: **(ver) boven benchmark**. Bekijk ratio/gap in tooltip.")

        bar = (
            alt.Chart(plot)
            .mark_bar(color=PFM_AMBER, cornerRadiusEnd=4)
            .encode(
                x=alt.X("score:Q", title=f"Score {compare_to} (0–100)", scale=alt.Scale(domain=[0, 100])),
                y=alt.Y("metric:N", sort="-x", title=None),
                tooltip=[
                    alt.Tooltip("metric:N", title="Metric"),
                    alt.Tooltip("score:Q", title="Score", format=".0f"),
                    alt.Tooltip("ratio:Q", title="Ratio vs benchmark (%)", format=".1f"),
                    alt.Tooltip("gap:Q", title="Gap vs benchmark (%) (+ = onder)", format=".1f"),
                    alt.Tooltip("store_value:Q", title="Store value", format=",.2f"),
                    alt.Tooltip("benchmark_value:Q", title="Benchmark value", format=",.2f"),
                ],
            )
    ...
    if show_quadrant:
        ...
        if quad.empty or quad["svi_v2"].dropna().empty:
            ...
            st.caption("Y-as is proxy: SPV vs company benchmark (100 = gelijk).")
